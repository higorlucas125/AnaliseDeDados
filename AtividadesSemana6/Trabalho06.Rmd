...
```
Hipótese nula (H0):  A variância da população é diferente a sigma_0\^2 (por exemplo, sigma\^2 != 1).
...
amostra1 <- c(14, 16, 17, 18, 19, 19, 15, 13, 11, 12)
# amostra2 <- c(11, 13, 14, 13, 16, 14, 13, 15, 12, 13)
amostra2 <- c(12, 11, 14, 17, 15, 13, 19, 15, 13, 14)
...
Hipotese nula (H0): As variancias das duas populacoes sao diferentes 
...
set.seed(123)  # Para reprodutibilidade
maquina1 <- rnorm(15, mean = 50, sd = 8)
maquina2 <- rnorm(15, mean = 50, sd = 4)
maquina3 <- rnorm(15, mean = 50, sd = 3)

# Combinar os dados em um data frame
dados <- data.frame(
  Medidas = c(maquina1, maquina2, maquina3),
  Maquina = factor(rep(c("Maquina1", "Maquina2", "Maquina3"), each = 15))
)
